        ...
        <Card>
            ...
            <div>
                ...
                >
                    ...
                    <FormItem label="头像：">
                        <div class="avatar">
                            <Icon :size="52" type="person" v-if="!userForm.avatar"></Icon>
                            <img :src="userForm.avatar" v-else width="100">
                        </div>
                        <Upload
                            :action="`${this.$baseUrl}upload_images`"
                            :show-upload-list="false"
                            :format="['jpg','jpeg','png']"
                            :max-size="2048"
                            :headers="{Authorization: token}"
                            :on-success="uploadAvatarSuccess">
                            <div class="upload"><Button type="ghost" icon="ios-cloud-upload-outline">点击上传</Button></div>
                        </Upload>
                    </FormItem>
                    ...
                    <FormItem label="登录密码：">
                        <Button size="small" @click="showEditPassword" type="warning">修改密码</Button>
                    </FormItem>
                    <!-- <div>
                        <Button style="width: 100px;" @click="cancelEditUserInfor">取消</Button>
        ...
        <Modal v-model="editPasswordModal" :closable='false' :mask-closable=false :width="500">
            ...
            <Form ref="editPasswordForm" :model="editPasswordForm" :label-width="100" label-position="right" :rules="passwordValidate">
                ...
                <FormItem label="新密码" prop="newPass">
                    <Input v-model="editPasswordForm.newPass" placeholder="请输入新密码，最长20位字符" type="password"></Input>
                </FormItem>
...
export default {
    ...
    methods: {
        ...
        init () {
            ...
            this.userForm.department = Number(Cookies.get('access')) === 0 ? '超级管理员' : Number(Cookies.get('access')) === 1 ? '管理员' : '代理';

            this.$http.get('users/get_one_person', {params: {
        ...
        uploadAvatarSuccess (res, file, fileList) {
            this.userForm.avatar = this.$baseUrl + 'images/' + res.data.url;
            this.$http.post('users/modify_person_info', {
                avatar: this.userForm.avatar,
                username: Cookies.get('user')
            }).then(res => {
                this.$store.commit('setAvator', res.data.avatar);
                this.$Message.success(res.msg);
            }).catch(res => {
            });
        }
